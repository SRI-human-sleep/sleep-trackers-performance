ebe2sleep <- function(data=NA,idCol="subject",RefCol="reference",deviceCol="device",epochLenght=30,
  ...
  # sleep staging recoding
  if(staging==FALSE & length(stages)>2){ 
    stop("only two elements should be used in the stages argument when staging = FALSE,
         e.g., stages = c(wake = 0, sleep = 1)") }
  if(staging==TRUE){ stage.labs = c(0,1,2,3) } else { stage.labs = c(0,1) }
  data$ref <- as.integer(as.character(factor(data$ref,levels=as.numeric(stages),labels=stage.labs)))
  data$device <- as.integer(as.character(factor(data$device,levels=as.numeric(stages),labels=stage.labs)))
