groupDiscr <- function(data=NA,measures=c("TST_device","TST_ref"),size="reference",logTransf=FALSE,
  ...
  if(prop.bias == FALSE){
    ...
    out <- cbind(out, # updating output table
                 ...
                 bias=paste(round(b0,digits)," + ",round(b1,digits)," × ",ifelse(size=="mean","mean","ref"),sep=""),
  ...
  if(logTransf == FALSE){
    ...
    if(prop.bias==FALSE & heterosced==FALSE){
      ...
      out <- cbind(out, # updating output table
                   ...
                   LOA.lower = paste("bias - 2.46(",round(c0,digits)," + ",round(c1,digits)," × ",
                   ...
                   LOA.upper = paste("bias + 2.46(",round(c0,digits)," + ",round(c1,digits)," × ",
                                     ifelse(size=="mean","mean","ref"),")",sep=""),
    ...
    out <- cbind(out, # updating output table
                 ...
                 # lower LOA and CI
                 LOA.lower = paste("bias - ",size," × ",round(ba.stat$LOA.slope,digits)),
                 LOA.lower_CI = paste("bias - ",size," × [",round(ba.stat$LOA.slope.CI.lower,digits),
                                      ", ",round(ba.stat$LOA.slope.CI.upper,digits),"]",sep=""),
                 
                 # upper LOA and CI
                 LOA.upper = paste("bias + ",size," × ",round(ba.stat$LOA.slope,digits)),
                 LOA.upper_CI = paste("bias + ",size," × [",round(ba.stat$LOA.slope.CI.lower,digits),
                                      ", ",round(ba.stat$LOA.slope.CI.upper,digits),"]",sep="")) }
